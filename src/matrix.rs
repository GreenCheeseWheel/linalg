// Native library
use std::{fmt::Display, fmt::Formatter, ops};

// My crates
use crate::file_reader;
use crate::matrix_product::{self, gram_schmidt, matrix_product};
...
impl Matrix {
    ...
    pub fn add_row(&mut self, target: usize, row: usize, scalar: f64) -> bool {
        ...
        for i in 0..self.cols {
            self.data[(target - 1) * self.cols + i] +=
                scalar * self.data[(row - 1) * self.cols + i];
        }
